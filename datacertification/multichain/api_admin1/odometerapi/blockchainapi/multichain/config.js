...
var monitor_multichain = process.env.monitor_multichain;

setInterval(function () {
    getMonitorInfo(function (response) {
    })
}, monitor_multichain);
 ...
 * @param {*} callback 
 */
function getMonitorInfo(callback) {
    var multichain = initiateMultichain();
    if (!fs.existsSync(timenow+"monitor.csv")){
        writer = csvWriter({ headers: ["Time", "Mempoolsize","Mempoolbytes","blocks","currentblocksize","currentblocktx","difficulty","errors","genproclimit","networkhashps","pooledtx","hashespersec"]});
   }
       else{
    ...
    multichain.getMempoolInfo({
        ...
        if (error) {
            ...
            writer.pipe(fs.createWriteStream(timenow+"monitor.csv", {flags: 'a'}));
            writer.write({
