...
admin1='1RoeRNkeBfaY6qi1xt8fJiEjHiMpsk6vHBAYYM';
admin2='1KSdNyFijRYgdr5y7dqNcCsnrTmrJf8no3c4Ad';

totalresult=[];
for type in types:
    for machine in machines:
        for TransactionNo in TransactionNos:
          data = pd.read_csv("/home/cyril/Documents/LOG/Multichain/"+str(machine)+"/"+str(TransactionNo)+"loopthird.csv",low_memory=False)
          result=[];
          ...
          originalsorteddata=data.sort_values('Start')
          sortedblockdata = blockdata.sort_values('Height');
          ...
          for index in range(0,len(sortedblockdata)):
            ...
            if (index == 0):
              previousblocktimestamp = originalsorteddata.iloc[0].Start;
...
#meanblockgaslimit=sortedblockdata[['BlockGasLimit']].mean(axis=0).BlockGasLimit;
#medianblockgaslimit=sortedblockdata[['BlockGasLimit']].median(axis=0).BlockGasLimit;

#meanblockgasused=sortedblockdata[['BlockGasUsed']].mean(axis=0).BlockGasUsed;
#medianblockgasused=sortedblockdata[['BlockGasUsed']].median(axis=0).BlockGasUsed;
          ...
          result.insert(19, meandiffficulty);
          result.insert(20, mediandifficulty);
          result.insert(21, meanblocksize);
          result.insert(22, medianblocksize);
          result.insert(22, meanblocktransactions);
          result.insert(23, medianblocktransactions);
          result.insert(24, meantxpersec);
          result.insert(25, mediantxpersec);
          result.insert(26, totalblocksize);

          totalresult.append(result);

with open("/home/cyril/Documents/LOG/MULT_PROCESS/procmulti.csv", 'w') as csvfile:
  fieldnames = ['type', 'machine','TransactionNo','Mean_transaction_time','Median_transaction_time','Total_Transaction_Time','Transaction_per_sec','Transaction_exceeding_per_sec','Totalblocks','admin1validatedtransactions','admin2validatedtransactions','meaninterleavingblocktime','medianinterleavingblocktime','admin1minedblocks','admin2minedblocks','meanblocktimesincefirstblock','medianblocktimesincefirstblock','totalblocktimesincefirstblock','meandiffficulty','mediandifficulty','meanblocksize','medianblocksize','meanblocktransactions','medianblocktransactions','meantxpersec','mediantxpersec','totalblocksize']
  writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

  writer.writeheader()
  for row in totalresult:
   writer.writerow({'type':row[0], 'machine':row[1],'TransactionNo':row[2],'Mean_transaction_time':row[3],'Median_transaction_time':row[4],'Total_Transaction_Time':row[5],'Transaction_per_sec':row[6],'Transaction_exceeding_per_sec':row[7],'Totalblocks':row[8],'admin1validatedtransactions':row[9],'admin2validatedtransactions':row[10],'meaninterleavingblocktime':row[11],'medianinterleavingblocktime':row[12],'admin1minedblocks':row[13],'admin2minedblocks':row[14],'meanblocktimesincefirstblock':row[15],'medianblocktimesincefirstblock':row[16],'totalblocktimesincefirstblock':row[17],'meandiffficulty':row[18],'mediandifficulty':row[19],'meanblocksize':row[20],'medianblocksize':row[21],'meanblocktransactions':row[22],'medianblocktransactions':row[23],'meantxpersec':row[24],'mediantxpersec':row[25],'totalblocksize':row[26]})