...
def sendTransaction():
   ...
   def worker():
        while True:
            item = q.get()
            threadfunction(item,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,totalresult,session)
            q.task_done()
    #num_worker_threads
   for i in range(100):
...
def getTransaction():
   with open("direct_async_queue_batches_http.csv","w") as csvfile:
