...
def sendTransaction():
   ...
   while howManyLeft>0:
        ...
        for i in range((iter*batchSize),(batchSize*(iter+1))):
            threadinstance = Thread(target=threadfunction,args=(i,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,totalresult,session))
            threadsarray.append(threadinstance)

            #print (".", end="")    
        for eachthread in threadsarray:
            eachthread.start()
...
def threadfunction(increment,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,totalresult,session):
  result=[]
...
def getTransaction():
   with open("direct_async_batches_http.csv","w") as csvfile:
