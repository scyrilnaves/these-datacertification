...
async def sendTransaction():
   ...
   async with websockets.connect(geth_ws) as ws:
    for i in range (0,100000):
     task = asyncio.ensure_future(threadfunction(sem,i,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,ws))
     tasks.append(task)
    totalresult=asyncio.gather(*tasks)
    await totalresult
    return totalresult
...
async def threadfunction(sem,increment,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,ws):
  result=[]
