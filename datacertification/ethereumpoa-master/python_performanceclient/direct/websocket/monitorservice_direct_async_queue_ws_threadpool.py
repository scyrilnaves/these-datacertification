...
def sendTransaction(totalresult):
   ...
   with concurrent.futures.ThreadPoolExecutor(max_workers=10000) as executor: 
    future_response={executor.submit(threadfunction,item,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,totalresult,ws):item for item in range(0,100000)}
    concurrent.futures.wait(future_response,timeout=100,return_when=concurrent.futures.ALL_COMPLETED)
    for future in concurrent.futures.as_completed(future_response):
      totalresult.append(future.result())
...
def getTransaction():
   with open("direct_async_queue_ws.csv","w") as csvfile:
    fieldname=['Transactionid','Start','End','Lapse']
    writer=csv.DictWriter(csvfile,fieldnames=fieldname)
    writer.writeheader()
    totalresult=[]
    txarray=sendTransaction(totalresult)
    for row in txarray:
