...
def sendTransaction():
   ...
   with open('../../privkey/admin1/keystore/UTC--2019-10-29T07-35-39.686997980Z--995453f12fb74910e6d90e4f2773187927a6a4d9') as keyfile:
    encrypted_key = keyfile.read()
    private_key = web3.eth.account.decrypt(encrypted_key, 'nissan')
    for j in range(howManyLeft):
          tx = {'from': account,
                    'to' : Web3.toChecksumAddress(maincontractaddress),
                    'nonce':hex(totalnonce+j),
                    'gas' : gashex,
                    'gasPrice':gaspricehex,
                    'data' : data} 
          
          signedtransaction=web3.eth.account.signTransaction(tx,private_key)
          # convert hex bytes to hex
          signedtransactionArray.append(Web3.toHex(signedtransaction.rawTransaction))
   
   def worker():
        while True:
            item = q.get()
            threadfunction(item,totalresult,ws)
            q.task_done()
    #num_worker_threads
   ...
   while howManyLeft>0:
    ...
    for i in range((iter*batchSize),(batchSize*(iter+1))):
        q.put (signedtransactionArray[i])
...
def threadfunction(increment,totalresult,ws):
  result=[]
  starttime=int(round(time.time()))

  txParameters = increment
...
def getTransaction():
   with open("direct_async_queue_batches_ws_rawtx.csv","w") as csvfile:
