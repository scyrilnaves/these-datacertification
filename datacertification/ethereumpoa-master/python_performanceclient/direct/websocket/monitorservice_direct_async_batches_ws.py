...
def sendTransaction():
   ...
   while howManyLeft>0:    
        ...
        for i in range((iter*batchSize),(batchSize*(iter+1))):
            threadinstance = Thread(target=threadfunction,args=(i,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,totalresult,ws))
...
def getTransaction():
   with open("direct_async_batches_ws.csv","w") as csvfile:
