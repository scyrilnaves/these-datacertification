...
def sendTransaction():
   ...
   def worker2():
        while True:
            item = q.get()
            threadfunction(item,account,maincontractaddress,totalnonce,gashex,gaspricehex,data,totalresult,ws2)
            q.task_done()
   ...
   while howManyLeft>0:
    ...
    for i in range((iter*batchSize),(int(batchSize/2)*(iter+1))):
        q.put (i)
    #print ("%d items queued." % 10)

    for i in range(iter*(int(batchSize/2)),(batchSize*(iter+1))):
        q2.put (i)
...
def getTransaction():
   with open("direct_async_queue_batches_ws.csv","w") as csvfile:
