...
printf "$(hostname -I)\n">>log.csv
printf "TIME|USEDMEMORY|AVAILABLEMEMORY|SWAPUSED|CPULOAD1|CPULOAD2|CPULOAD3|PAC_REC|PAC_TRA|IOWAIT|IDLE|USED|CPU1used|CPU1iowait|CPU1idle|CPU2used|CPU2iowait|CPU2idle\n" >> log.csv
...
#Gateway Traffic
received=$(netstat -i | grep -e Iface -e ens160 |awk '(NR==2)'| awk '{print $4}')

transmitted=$(netstat -i | grep -e Iface -e ens160 |awk '(NR==2)'| awk '{print $8}')
...
printf "$dat|$usedmem|$availablemem|$swapused|$loadone|$loadtwo|$loadthree|$received|$transmitted|$iowait|$idle|$used|$mpstatoneused|$mpstatoneiowait|$mpstatoneidle|$mpstattwoused|$mpstattwoiowait|$mpstattwoidle\n" >> log.csv
