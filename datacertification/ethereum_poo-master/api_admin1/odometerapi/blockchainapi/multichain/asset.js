...
function getAdminforPermisssion(listAdminPermissions) {
    ...
    for (let i = 0; i < Object.keys(listAdminPermissions).length; i++) {
        ...
        multichain.getAddresses({
            ...
            for (let i = 0,p=Promise.resolve(); i < Object.keys(response).length; i++) {
                ...
                multichain.listStreamKeyItems({
                    ...
                    for (let j = 0,q=Promise.resolve(); j < Object.keys(listitemresponse).length; j++) {
...
function pollforAssetTransferRequesttoApprove(callback) {
    ...
    pendingAssetApprovals(function (pendingapprovals) {
        for (let i = 0,p=Promise.resolve(); i < Object.keys(pendingapprovals).length; i++) {
            p=p.then(_=> new Promise(resolve =>{
            multichain.listStreamKeyItems({
                stream: assetrequest,
    ...
    multichain.getAddresses({
        ...
        if (addresserr) {
            ...
            multichain.listStreamKeys({
                ...
                getUnique_MatchingItemKeys(listStreamKeysinfo, addrinfo[0],function(effectiveApprovals){
...
function getUnique_MatchingItemKeys(listStreamKeys, address,callback) {
    var PromiseArray=[];
    var uniqueMatcheditems = [];
    let j = 0;
    if(listStreamKeys!=null && listStreamKeys!=undefined){
    for (let i = 0,p=Promise.resolve(); i < Object.keys(listStreamKeys).length; i++) {
        PromiseArray.push(p);    
      p = p.then(_=> new Promise(resolve =>{
...
Promise.all(PromiseArray).then(() =>  {return callback(uniqueMatcheditems);});
...
function pollForAssetApprovals(transactionkey, callback) {
    ...
    multichain.listStreamKeyItems({
        ...
        calculateAssetApproval(function (resp) {
            checkDuplicateSignatures(listStreamKeysinfo,resp,function(valid,finaldatawithsign) {
                if(valid){
 ...
 */
 ...
 function checkDuplicateSignatures(listStreamKeysinfo,resp,callback){
     var adminAddressArray=[];
     let signcount=0;
     var metadatafiltered=[];
     if(Object.keys(listStreamKeysinfo).length >= resp){
        for (let i = 0; i < Object.keys(listStreamKeysinfo).length; i++) {
            //Add the admin who have signed the message
            if(!(adminAddressArray.includes(listStreamKeysinfo[i].data.json.adminaddress))){
                adminAddressArray[signcount] = listStreamKeysinfo[i].data.json.adminaddress;
                metadatafiltered[signcount] = listStreamKeysinfo[i].data;
                signcount++;
            }
        }
        if((signcount)>=resp){
            return callback(true,metadatafiltered);
        }
        else{
    ...
    multichain.getAddresses({
        ...
        if (addresserr) {
            ...
            multichain.getAddressBalances({
                ...
                for (let i = 0; i < Object.keys(addressBalanceresponse).length; i++) {
